    ...
    private void startGame() {
        ...
        if(gameState == 1) {
            new Thread() {
                public void run() {
                    String save;
                    ServerSocket server = Gdx.net.newServerSocket(Protocol.TCP, connectUrl.split(":")[0], Integer.parseInt(connectUrl.split(":")[1]), null);
                    Socket client;
                    while(true) {
                        try { client = server.accept(null); break; } catch(Exception ex) {}
                    }
            ...
            new Thread() {
                public void run() {
                    Socket client = Gdx.net.newClientSocket(Protocol.TCP, connectUrl.split(":")[0], Integer.parseInt(connectUrl.split(":")[1]), null);
                    BufferedReader stream = new BufferedReader(new InputStreamReader(client.getInputStream(), StandardCharsets.UTF_8));
                    while(true) {
                        try {
                            if(selected == -1) {
